...
def fetch_events(body_names, date_from, date_to):
    ...
    params = {
        ...
        "$top": 30,
        "$select": "EventId,EventBodyName,EventDate,EventTime,EventLocation,EventAgendaStatusName",
        "token": LEGISTAR_TOKEN,   # ← add this line
    }
...
@app.route("/debug")
def debug():
    return jsonify({"token_set": bool(LEGISTAR_TOKEN), "token_length": len(LEGISTAR_TOKEN)})
